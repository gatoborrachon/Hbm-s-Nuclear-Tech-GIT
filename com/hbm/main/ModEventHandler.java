...
public class ModEventHandler {
	...
	public void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event) {
		if (showMessage) {
			event.player.addChatMessage(new ChatComponentText(
					"Loaded world with Hbm's Nuclear Tech Mod " + RefStrings.VERSION + " for Minecraft 1.7.10!"));
		}
	...
	public void worldTick(WorldTickEvent event) {
		...
		for(Object ep : event.world.playerEntities){
			if(ep instanceof EntityPlayer && !((EntityPlayer) ep).inventory.hasItem(ModItems.stealth_boy)){
				((EntityPlayer) ep).getEntityData().setBoolean("StealthActivated", false);
				PacketDispatcher.wrapper.sendToAll(new StealthBoyPacket(false, (EntityPlayer) ep));
			}
		}
		}
	...
	/*
	 * @SubscribeEvent public void itemSmelted(PlayerEvent.ItemSmeltedEvent e) {
	...
	/*
	 * @SubscribeEvent public void itemCollected(PlayerEvent.ItemPickupEvent e)
